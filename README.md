 ...
 **请勿使用本仓库直接部署**  
 ...
 7. HEROKU_API_KEY, HEROKU_APP_NAME, HEROKU_RESTART_TIME三个变量用于固定Heroku dyno重启时间。Heroku的重启周期是24-27小时左右，这三项变量设置好以后可以把Heroku dyno重启周期固定到24小时，避免出现随机重启的情况。如果通过Cloudflare Workers反代实现单双日切换，此三项不需要填写，Workers将通过指定的时区定时唤醒单双日指定的dyno，重置24小时重启周期。
